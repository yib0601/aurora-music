...
一条音源可同时提供两种能力，搜索与歌单解析共用同一条配置、同一个启用开关。音源有两种填法：

**标准音源（默认）**：只填两栏——**服务地址**（如 `https://music.lighthouses.top`）+ **密钥**（服务端 `API_KEY`），搜索与歌单解析接口由应用自动生成：
...
- 应用先读服务端的**端点自描述**（`GET {服务地址}/` 的 `endpoints.search` / `endpoints.playlist`）再组装；服务端改路径或参数名时客户端无需改配置，读不到才用上面的默认约定。
- 「测试连接」会请求根路径与 `/health`，确认服务在线、密钥有效，并展示组装出的两个地址。
- 密钥输入框支持直接粘贴**完整接口地址**（含 `{query}` 占位符与 `key=`，例如迁移前的老配置），应用会自动拆成服务地址与密钥两栏。

**自定义接口**：手写完整地址与占位符，适用于第三方接口或子路径部署，规则如下：
...
> 同一份配置在两种填法间可互转：切到「自定义接口」会把组装结果写进地址栏，切回标准音源则自动拆回两栏。
